function dmrs_simulation()
    ...
    for port = 1:num_ports
        ...
        elseif dmrs_type == 2
            ...
            resource_grid(subcarriers+1, sym+1) = dmrs_sequences(port,:);
        end
    end
    ...
    for port = 1:num_ports
        ...
        for sym = dmrs_symbols
            ...
            % 标记 DMRS 符号
            plot(sym+1, dmrs_sc+1, '.', 'Color', color, 'MarkerSize', 10); % 使用红色点标记 DMRS 符号
        end
    end
    % 标记非 DMRS 符号
    non_dmrs_symbols = setdiff(0:num_symbols-1, dmrs_symbols);
